...
def generate_testfile(time_window_range:tuple, travel_time_range:tuple, N=100, time_window_dist='uniform', travel_time_dist='uniform', filename='test1.txt'):
    '''
    This function generates test cases for the TSP with Time Window problem. The parameters are:
        ...
        + time_window_dist and travel_time_dist are strings which specify the type of distribution you want to use when elements of the time window and travel time matrices are generated randomly
        + filename is the name of the file that stores the generated test case
    '''
    file_handle = open(filename, 'w+')
    if time_window_dist == 'uniform':
        print('Good')
        time_windows = np.random.randint(0, sum(time_window_range)//2, size=(N, 2))
        print(time_windows)
        time_windows[:, 1] = np.random.randint(time_windows[:, 0], time_window_range[1], size=N)
        print(time_windows)
    elif time_window_dist == 'normal':
        mean = sum(time_window_range)//4
        std_dev = sum(time_window_range)//8
        time_windows = np.round(np.column_stack([
            np.clip(np.random.normal(mean, std_dev, N), 0, time_window_range[1]),
            np.zeros((N, 1))
        ])).astype(int)
        time_windows[:, 1] = np.random.randint(time_windows[:, 0], time_window_range[1], size=N)
    service_times = np.random.randint(1, time_window_range[1]//(2*N), size=N)

    if travel_time_dist == 'uniform':
        travel_times = np.random.randint(travel_time_range[0], travel_time_range[1], size=(N+1, N+1))
        np.fill_diagonal(travel_times, 0)
        travel_times = np.divide((travel_times + travel_times.T), 2) 
    elif travel_time_dist == 'normal':
        mean = sum(travel_time_range)//2
        std_dev = sum(travel_time_range)//4
        travel_times = np.random.normal(mean, std_dev, size=(N+1, N+1))
        travel_times = np.abs(travel_times.round())
        np.fill_diagonal(travel_times, 0)
    ...
    for i in range(N):
        file_handle.write(f'{time_windows[i][0]} {time_windows[i][1]} {service_times[i]}\n')
