...
df = pd.DataFrame(columns=['Test_Case', 'Path', 'Value', 'Time'])

folder = "E:\\Di hoc\\Dai Cuong\\May mon IT ngu ngu\\Toi uu hoa\\Mini Project\\test_testcase"
files = os.listdir(folder)

for filename in files:
    print(filename)
    start_time = time.time()
    solution_found_time = 0
            ...
            for i in range (0, n - 1):
                for j in range (i + 1, min(i + vertex_check, n)):
                    ...
                    if travel_time < cur_MIN:
                        ...
                        cur_MIN = travel_time 
                        solution_found_time = time.time() - start_time
    ...
    df = pd.concat([df, pd.DataFrame([{'Test_Case': filename, 'Path': optimal_path, 'Value': Calculate(optimal_path), 'Time': solution_found_time}])], ignore_index=True)

df.to_excel('Checker1.xlsx', index = False)