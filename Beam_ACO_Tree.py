...
import numpy as np
import time
from test_generator import read_input_file
...
def Calculate(path:List[int]):
    '''
    This function calculates the travel time cost of a path.
    Return: The travel time cost, if the path violates any time constraints, the cost will be a very big integer
    '''
    cur_pos = 0
    total_time = 0
    travel_time = 0
...
def lex_compare(path1:List[int], path2:List[int]) -> bool:
    '''
    This function compares two path based on their number of violations and travel times
    Return: True if path1 is "better" than path2, False otherwise
    '''
    if path2 == None:
        return True
...
def LocalSearch(start_path:List[int], enhancement:bool=False, allowed_time:int=None, accepted_delay:int=None):
    '''
    LocalSearch local search to enhance a starting path by finding its nearest neighbors
        + start_path: a permutation from 1 to n representing the current path
        + enhancement: a boolean variable that decides whether to use the paths found during the execution of the algorithm to update the pheromone matrix
        + allowed_time: the time that tabu is allowed to run
    Return: a permutation from 1 to n representing a potentially better path
    '''
    tabu = deque()
...
def calculate_heuristic(parent:Node, new_value:int):
    ...
    return (heuristic_value, 0 (rank sum is calculated later), the time passed before servicing at point new_value, count of violations, total travel time to reach point new_value)
    '''
...
class Tree:
    ...
    - Every instance of this class will have 3 main parameters for the PBS:
        ...
        + q0: The probability that the nodes are chosen deterministically based on heuristic values after each step
    '''
    ...
    def shrink(self):
        '''
        Sort the leaves in ascending order of the number of violations and if they are equal, sort according to the rank sum and travel time and only keep k_bw best leaves for future expansion
        '''
...
class BeamSolver:
    ...
    - Every instance of this class will have 4 main parameters for the PBS:
        + k_bw (int): this is the beam width: the maximum number of nodes kept after each shrinking step for further expansion
        + muy (float): the expansion rate. If there are currently L leave nodes, after the expansion step it will be L*muy
        + N_s (int): the number of stochastic samples taken for partial evaluation
        + q0: The probability of deterministic tree expansion
    '''
    def __init__(self, k_bw:int, muy:float, N_s:int, q0:float=0.5):
        self.k_bw = k_bw
        self.muy = muy
...
def ApplyPheromoneUpdate(good_paths:List[List], weights:List[float]=None, ro:float=0.1):
    '''
    This function updates the pheromone matrix using the following parameters:
        + good_paths is a list of paths that are used to enhance the pheromone trails
        + weights is a list of weights corresponding to each path used
...
def ConvergenceFactor() -> float:
    '''
    Returns the convergence factor of the pheromone matrix
    '''
    current_sum = 0
    for i in range(n+1):
        for j in range(n+1):
